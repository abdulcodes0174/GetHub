...
vector<string> TestControl::menu ={
    "RepoList test",
    "GetHub repo test",
    "GetHub file test\n  (5 marks total if tests 1-3 still work)\n\nClient tests:",
    "Client download test (10 marks)",
    "Client memory test (10 marks)\n",
...
void TestControl::launch(){
    ...
    while (choice!= 0){
        ...
        switch(choice){
            ...
            case 6: int mark = 5;
                    // TODO adjust these marks - this should be a
                    // deduction if the tests fail
                    mark -= (10-repoListTest());
                    mark -= (10-getHubRepoTest());
                    mark -= (10-getHubFileTest());
                    mark = mark < 0 ? 0 : mark;
                    cout<<endl<<"Your mark is "<<mark<<"/5 for the GetHub tests"<<endl;

                    mark += clientDownloadTest();
                    mark += clientMemoryTest();
                    cout<<endl<<"Your mark is "<<mark<<"/25"<<endl;
                    break;
        }
    }
}
...
int TestControl::getHubFileTest(){
    ...
    cout<<"Checking output for correct Files and Dates"<<endl;
    errors = 0, errors2 = 0;
    tester.find({2,0,3}, fileTitles[2], errors);
    tester.find({2,0,3}, dates[2], errors2);
    if (errors != 0){
        cout<<"Error: output is incorrect"<<endl;
    }else if (errors2 != 0){
        cout<<"Error: dates not found"<<endl;
    ...
    tester.confirmAbsent({1,4}, fileTitles[2], errors);
    tester.confirmAbsent({1,4}, dates[2], errors2);
    if (errors != 0){
    ...
    cout<<"Testing printFileContents file"<<endl;
    cout<<"Printing file 1 from repo 2"<<endl;
    tester.initCapture();
    getHub.printFileContents(2,1);
    tester.endCapture();

    cout<<"Checking output"<<endl;
    tester.find({0}, content[2], errors);
    if (errors != 0){
...
int TestControl::clientDownloadTest(){
    cout<<"Client cloneRepo test"<<endl;
    GetHub getHub;
    initGetHub(getHub);
    Client client;
    int marks = 0;

    cout<<"Attempting to print Repo"<<endl;
    tester.initCapture();
    client.printRepo();
    tester.endCapture();
    ...
    if (errors != 0){
        ...
        cout<<"No repos found, output is correct"<<endl;
        marks += 2;
    }

    cout<<"Downloading Repo 3"<<endl;
    client.cloneRepo(getHub, 3);

    cout<<"Printing Repo"<<endl;
    tester.initCapture();
    client.printRepo();
    tester.endCapture();

    cout<<"Checking for Repo title"<<endl;
    errors = 0;
    tester.find({3}, repoTitles, errors);
    if (errors != 0){
        cout<<"Error: repo not found"<<endl;
    }else{
        cout<<"Repo title found, output is correct"<<endl;
        marks += 2;
    }

    cout<<"Checking for File names"<<endl;
    errors = 0;
    tester.find({0,1,2,3,4}, fileTitles[3], errors);
    if (errors != 0){
        cout<<"Error: file names not found"<<endl;
    }else{
        cout<<"File names found, output is correct"<<endl;
        marks += 2;
    }

    cout<<"Printing file 2"<<endl;
    tester.initCapture();
    client.printFileContents(2);
    tester.endCapture();

    cout<<"Checking output"<<endl;
    tester.find({0}, content[3], errors);
    if (errors != 0){
        cout<<"Error: file content not found"<<endl;
    }else{
        cout<<"File content found, output is correct"<<endl;
        marks += 2;
    }

    cout<<"Downloading Repo 1"<<endl;
    client.cloneRepo(getHub, 1);

    cout<<"Printing Repo"<<endl;
    client.printRepo();


    cout<<"Printing File 4"<<endl;
    tester.initCapture();
    client.printFileContents(4);
    tester.endCapture();

    cout<<"Checking output"<<endl;
    tester.find({4}, content[1], errors);
    if (errors != 0){
        cout<<"Error: file content not found - are your files in order by Date?"<<endl;
    }else{
        cout<<"File content found, output is correct"<<endl;
        marks += 2;
    }

    cout<<"Client cloneRepo test complete"<<endl;
    cout<<"Marks: "<<marks<<"/10"<<endl;

    // for (int i = 0; i < 5; ++i){
    //     cout<<i<<") "<<fileTitles[1][i]<<endl;
...
int TestControl::clientMemoryTest(){
    ...
    cout<<endl<<"Downloading Repo 3"<<endl;
    client.cloneRepo(getHub, 3);

    cout<<endl<<"Deleting Repo 3 from GetHub"<<endl;

    getHub.deleteRepo(3);

    cout<<endl<<"Printing Repo 3 from Client"<<endl;
    tester.initCapture();
    client.printRepo();
    tester.endCapture();

    cout<<endl<<"Checking for Repo title"<<endl;
    int errors = 0;
    tester.find({3}, repoTitles, errors);
    if (errors != 0){
        cout<<"Error: repo not found"<<endl;
    ...
    cout<<endl<<"Checking for File names"<<endl;
    errors = 0;
    tester.find({0,1,2,3,4}, fileTitles[3], errors);
    if (errors != 0){
        cout<<"Error: file names not found"<<endl;
    ...
    cout<<endl<<"Downloading Repo 1, then Repo 0"<<endl;
    client.cloneRepo(getHub, 1);
    client.cloneRepo(getHub, 0);

    cout<<endl<<"Printing Repo 1 from GetHub"<<endl;
    tester.initCapture();
    getHub.printRepo(1);
    tester.endCapture();
    ...
    cout<<endl<<"Checking for Repo title"<<endl;
    errors = 0;
    tester.find({1}, repoTitles, errors);
    if (errors != 0){
        cout<<"Error: repo not found"<<endl;
    }else{
        cout<<"Repo title found, output is correct"<<endl;
    ...
    cout<<endl<<"Checking for File names"<<endl;
    errors = 0;
    tester.find({0,1,2,3,4}, fileTitles[1], errors);
    if (errors != 0){
        cout<<"Error: file names not found"<<endl;
    }else{
        cout<<"File names found, output is correct"<<endl;
...
const string TestControl::fileTitles[5][5] = {
